...
public class Validator {
	...
	public static int getInt(Scanner scnr, String prompt, int min, int max) {
		...
		do {
			...
			if (number < min) {
				isValid = false;
				System.out.print("The number must be at least " + min);
				System.out.println("Please try again.");
			} else if (number > max) {
				isValid = false;
				System.out.print("The number must not be larger than " + max);
				System.out.println("Please try again.");
			} else {
				isValid = true;
				if (number == 1) {
...
//					
//					Country s = new Country(scnr.nextLine());
//
//					Country.setCountryName(scnr.nextLine());
//					CountriesApp.writeToFile(directoryFolder, fileName);
//					System.out.println();
//					System.out.println("Here's the revised country list:");
//					CountriesApp.readFromFile(directoryFolder, fileName);
				} else {
					System.out.println("\nGoodbye.");
				}
			}
